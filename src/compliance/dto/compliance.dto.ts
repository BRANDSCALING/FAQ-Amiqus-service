...
export class InitAmiqusDto {
  ...
  /**
   * Optional. Written to the Amiqus client's `phone_number` field. Not shown
   * on the identity "Your details" step, but stored on the client record.
   */
...
export class InitDocuSealDto {
  ...
  /**
   * Optional. Used to pre-fill any phone/mobile field the SLA template
   * exposes. Safe to omit — unmatched values are simply not sent.
   */
  @ApiPropertyOptional({ example: '+447700900123' })
  @IsOptional()
  @IsString()
