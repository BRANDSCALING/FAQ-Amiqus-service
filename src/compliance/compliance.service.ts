...
export class ComplianceService {
  ...
  /**
   * Build the prefill `fields` for the combined SLA template (4) by
   * introspecting the template's ACTUAL field labels at runtime and
   * keyword-matching each to a known value. This avoids hardcoding labels
   * for a template whose field names we can't see from outside the service.
   *
   * Never throws: on any error (fetch failure, unexpected shape) it returns
   * [] so the submission is still created — the existing "unknown field"
   * fallback in initDocuSeal is the second safety net. All matched fields
   * are logged so the mapping can be verified/tightened after deploy.
   */
  private async buildSlaPrefillFields(
    templateId: number,
    dto: InitDocuSealDto,
    today: string,
    ...
    try {
      ...
      if (res.status < 200 || res.status >= 300) {
        this.logger.warn(
          `SLA prefill: GET template ${templateId} returned ${res.status}; creating submission without prefills`,
        );
        return [];
      }
      ...
      for (const f of fields) {
        ...
        // readonly: the value is authoritative (it comes from the user's
        // verified profile), so lock it. Readonly prefilled fields are NOT
      ...
      this.logger.log(
        `SLA prefill: matched ${out.length}/${fields.length} fields on template ${templateId}: ${
          out.map((o) => o.name).join(' | ') || '(none)'
        }`,
      );
      return out;
    } catch (e: unknown) {
      this.logger.warn(
        `SLA prefill build failed (non-fatal): ${(e as Error)?.message}; creating submission without prefills`,
      );
      return [];
    }
  }
  ...
  /**
   * Keyword-match one DocuSeal field label to a known value. Order matters:
   * more specific rules (company reg number, company name, email, phone,
   * date, address) are tested before the generic person-name fallback.
   * Returns null when no rule fits (field left for the user to complete).
   */
  private matchSlaFieldValue(
    rawName: string,
    type: string,
    dto: InitDocuSealDto,
    today: string,
    ...
    // Signatory title. On this deed the HSP signs "acting by a director"
    // (and the counterparty block is likewise "Title: Director"), so this is
    // the signatory's ROLE, not an honorific — we default it to "Director"
    // rather than guessing Mr/Miss from the name (which would be unreliable
    // and could misgender someone on a legal document).
    if (/\btitle\b/.test(n)) return 'Director';
    // Date
    if (type === 'date' || /\bdate\b/.test(n)) return today;
    // Email
    if (/e-?mail/.test(n)) return dto.hspEmail;
    // Phone / mobile
    if (/(phone|mobile|telephone|\btel\b|contact number)/.test(n)) return phone || null;
    // Company registration number (has company/registration + number, not a name)
    if (
      /(company|registration|reg\.?\b|companies house)/.test(n) &&
      /(number|no\.?\b|reg\b)/.test(n) &&
      !/name/.test(n)
    ) {
      return dto.companyRegNumber;
    }
    // Company / business name (name of an entity, not a person)
    if (/(company|business|organisation|organization|trading|firm)/.test(n) && /name/.test(n)) {
      return dto.companyName;
    }
    // Address (registered office / property / company address)
    if (/address/.test(n)) return dto.registeredAddress;
    // Person name (director / signatory / first party / HSP / full / print / your name)
    if (/name/.test(n)) return dto.hspName;
    return null;
  }
  ...
  async initAmiqus(dto: InitAmiqusDto): Promise<{ performUrl: string; recordId: number; clientId: number }> {
    ...
    // NOTE: Amiqus's POST /clients does NOT accept a phone_number field —
    // sending one returns 422 "This property is not supported". Phone is
    // captured on our side (users.phone_num) and used only for the DocuSeal
    // SLA prefill, never sent to Amiqus. dto.phoneNumber is intentionally
    // ignored here.
    try {
  ...
  async initDocuSeal(dto: InitDocuSealDto): Promise<{ slug: string; submissionId?: number }> {
    ...
    // HSPSLA (template 1) and TENANTS (template 2) use hardcoded field
    // names below. The combined SLA template (template 4,
    // Allianz_Housing_Complete_Contracts_1) is pre-filled by introspecting
    // its ACTUAL field labels at runtime and keyword-matching them to our
    // values (see buildSlaPrefillFields) — so we don't have to hardcode
    // labels we can't see. Any field that doesn't match a rule is left for
    // the user; on a DocuSeal validation error the fallback path below still
    // creates the submission with no prefills.
    const slaFields =
      dto.contractType === 'SLA'
        ? await this.buildSlaPrefillFields(templateId, dto, today)
        : [];
    const hspFields =
      dto.contractType === 'SLA'
        ? slaFields
